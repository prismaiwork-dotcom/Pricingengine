...
// ── AI ANALYSIS ───────────────────────────────────────────────────────────
async function getAIAnalysis(form, p) {
...
PRICING:
Unit: $${p.unitPrice} | Range: $${p.low}-$${p.high} | Total: $${p.total}
Confidence: ${p.confidence} | Matches: ${p.matchCount} | Escalate: ${p.escalate}

Respond ONLY in this exact format, no markdown, no asterisks:

RECOMMENDATION
$${p.unitPrice}/unit — $${p.total} total for ${form.quantity} units — ${p.confidence} confidence
...
OWNER ACTION
${p.escalate ? "YES — Owner approval required before sending." : "NO — Can be sent directly without owner approval."}`;
